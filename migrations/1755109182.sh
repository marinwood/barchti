...
# Users can still opt-in to Cloudflare DNS using: barchti-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-barchti-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-barchti-dns.network
  ...
  echo "To use Cloudflare DNS, run: barchti-setup-dns Cloudflare"
fi