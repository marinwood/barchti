...
barchti-refresh-config uwsm/default
barchti-refresh-config uwsm/env
barchti-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, barchti-launch-editor' ~/.config/hypr/bindings.conf
fi
