...
for pkg in "${PACKAGES[@]}"; do
  if ! barchti-pkg-present "$pkg"; then
...
if $NEEDS_MIGRATION; then
  ...
  barchti-pkg-drop walker-bin walker-bin-debug

  barchti-pkg-add "${PACKAGES[@]}"

  source $BARCHTI_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  barchti-refresh-config walker/config.toml
  barchti-refresh-config elephant/calc.toml
  barchti-refresh-config elephant/desktopapplications.toml
fi
